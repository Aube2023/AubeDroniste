...
CREATE TABLE IF NOT EXISTS pilot_profiles (
    ...
    kind              TEXT NOT NULL DEFAULT 'pro', -- pro | recreational | school (onglets annuaire)
    school_programs   TEXT,                       -- ecoles : formations proposees (texte, une par ligne)
...
-- Collectes « Soutenez ce pilote » : un pilote verifie dit quel materiel il
-- veut financer et pourquoi ; chaque contribution payee lui est versee
-- (moins CAMPAIGN_FEE_PCT). Financement souple : pas de tout-ou-rien.
...
-- ---------------------------------------------------------------------------
-- AubeBeacon : balises de telemetrie posees sur les drones.
-- Source unique du DDL : beacon/schema.py (repris par db.run_migrations pour
-- la prod existante) ; cette copie sert aux bases neuves et a la lecture.
-- ---------------------------------------------------------------------------
